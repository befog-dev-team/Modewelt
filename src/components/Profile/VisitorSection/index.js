...
export default function VisitorSection({ visitors }) {
    return (
        <div>
            <div className="min-h-[300px] bg-[#ffffff] dark:bg-gray-900 mt-5 shadow-lg p-4 border dark:border-gray-800 transition-colors">
                <div className="flex justify-between px-3 py-3 w-full font-[Gotham] mt-2">
                    <span className="font-semibold text-sm dark:text-gray-200">VISITORS</span>
                    {/* <span
                ...
                <hr className="w-full mx-auto my-2 border-t border-gray-300 dark:border-gray-800" />
                    ...
                    {visitors.map((visitor) => (
                        <div key={visitor.id} className="flex items-center w-full h-[52px] space-x-3">
                            ...
                            <div className="flex flex-col space-y-1">
                                <Link href={`/profile/${visitor.username}`} prefetch={true}>
                                    <span className="font-bold text-sm hover:underline dark:text-gray-100">{visitor.displayName}</span>
                                </Link>
